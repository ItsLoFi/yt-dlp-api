...
class CookieService:
    ...
    async def _test_youtube_authentication(self) -> bool:
        """
        Validate the YouTube cookie file without performing
        an active authentication request.

        The actual yt-dlp download will determine whether
        YouTube accepts the cookies.
        """
        if self._test_mode:
        ...
        # Netscape-format validation was already performed by
        # validate_cookie_file().
        #
        # Do not use a fixed YouTube video as an authentication
        # probe because YouTube may reject automated requests
        # independently of cookie validity.
        #
        # The real download operation will determine whether
